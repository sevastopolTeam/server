...
void RegisterUserHandler(TDataSource& dataSource, const httplib::Request& req, httplib::Response& res) {
    try {
        TRecordEnglishUser user(NJson::TJsonValue::parse(req.body));
        NJson::TJsonValue result;
        if (user.IsValide(&result)) {
            if (dataSource.CollectionEnglishUser.IsAlreadyRegistred(user)) {
                result["status"] = "user already register";
            } else {
                if (!dataSource.CollectionEnglishUser.Register(user)) {
